...
among the polytopes contained in it. It's \textit{underlying set} $|\mathrak{C}|$ is the union of all of 
its polytopes $  |\mathrak{C}| \coloneqq \cup_{F\in \mathfrak{C}} F.$
...
cycle used by paths. The triangulation of the clusters in $G'_m$ is desribed by
...
Let $G''_m$ be the graph of a $(d-4)$-fold prism over $\NC_4(m)''$ for $d\geq 4$. 
Any separator 
$(A,B,C)$ of $\NC_4(m)''$ can be turned into a separator of $G''_m$ by setting 
...
far away from expanding either. The key property is taking a fairly good expander (an $m$-cube)
and modifying it so that it becomes a simple polytope (and therefore a regular graph). This
is done by replacing the vertices with 3-regular graphs. There is a systematic way of doing this, called
...
The important part is that if $G$ and $H$ are good expanders, then $G \circ H$ is too. 
